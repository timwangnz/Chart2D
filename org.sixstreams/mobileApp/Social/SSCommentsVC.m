...
-(void) onDataReceived:(id)objects
{
    ...
    {
        self.view.frame = CGRectMake(0, 0 , self.view.frame.size.width,190);
    }
    
    vComments.hidden = NO;
    btnAddComment.center = CGPointMake(self.view.center.x, 170);
    id item = [objects objectAtIndex:0];
    self.lCount.text = [NSString stringWithFormat:@"Total %d Reviews", [objects count]];
...
- (void) forceRefresh
{
    if(self.itemCommented && [self.itemCommented objectForKey:REF_ID_NAME])
    {
        [self.predicates removeAllObjects];
        [self.predicates addObject:[SSFilter on:COMMENTED_ON op:EQ value:[self.itemCommented objectForKey:REF_ID_NAME]]];
        [super forceRefresh];
    }
}
