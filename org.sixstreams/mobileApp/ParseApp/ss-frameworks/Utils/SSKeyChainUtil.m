...
@interface SSKeyChainUtil (PrivateMethods)
...
@implementation SSKeyChainUtil

@synthesize keychainItemData, genericPasswordQuery;

static SSKeyChainUtil *util;

+ (SSKeyChainUtil *) util
{
    if (!util)
    {
        util = [[ SSKeyChainUtil alloc]initWithIdentifier: //@"com.sixstreams"
                [NSString stringWithFormat: @"com.sixstreams.%@", [SSApp instance].name]
                                              accessGroup:nil];
    }
    return util;
}
...
{
    [[self util] resetKeychainItem];
}

+ (NSString *) authSource
{
   return [[self util] objectForKey:(__bridge id)(kSecAttrService)];
}

+ (NSString *) username
{
   return [[self util] objectForKey:(__bridge id)(kSecAttrAccount)];
}

+ (NSString *) password
{
    return [[self util] objectForKey:(__bridge id)(kSecValueData)];
}

+ (void) saveUsername:(NSString *) username authSource:(NSString *) authSource password:(NSString *) password
{
    [[self util] setObject:password forKey:(__bridge id)(kSecValueData)];
    [[self util] setObject:username forKey:(__bridge id)(kSecAttrAccount)];
    [[self util] setObject:authSource ? authSource : [NSString stringWithFormat: @"com.sixstreams.%@", [SSApp instance].name] forKey:(__bridge id)(kSecAttrService)];
}
