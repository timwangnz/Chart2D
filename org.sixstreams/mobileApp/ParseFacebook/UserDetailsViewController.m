...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *logoutButton = [[UIBarButtonItem alloc] initWithTitle:@"Log Out" style:UIBarButtonItemStylePlain target:self action:@selector(logoutButtonTouchHandler:)];
...
- (void)updateProfile {
    ...
    if ([[PFUser currentUser] objectForKey:@"profile"][@"pictureURL"]) {
        ...
        if (!urlConnection) {
            DebugLog(@"Failed to download picture");
        }
    }
}
