...
public class SecurityService extends DefaultService {
    ...
    private Person getProfile(String username) {
        ...
        try {
            ...
            if (clazz == null) {
                sLogger.log(Level.WARNING, "Failed to login user {0}, can not load person class {1}", new Object[]{username, profileClass});
                return null;
            }
            List<? extends Person> objects = pm.query(filters, clazz, 0, 10);
            if (objects != null && objects.size() > 0) {
                if (objects.size() > 1) {
                    sLogger.log(Level.WARNING, "Found more than one user with the same user name, should not hannen, {0} of {1}", new Object[]{username, profileClass});
                }
                Person person = (Person) objects.get(0);
                GoodCache.getCache(profileClass).put(username, person);
                return person;
            } else {
                sLogger.log(Level.WARNING, "Failed to login user - not found {0} as {1}", new Object[]{username, profileClass});
                return null;
            }
    ...
    private User getUserByUsername(String username, String authSource) {
        ...
        try {
            ...
            if (clazz == null) {
                sLogger.log(Level.WARNING, "Failed to login user {0}, can not load user ", username);
                return null;
            }
            ...
            if (object != null) {
                ...
                try {
                    ...
                    try {
                        ...
                        sLogger.log(Level.INFO, "An user and profile has been created {0}", profile);

                    } catch (IOException e) {

                        sLogger.log(Level.SEVERE, "Failed to create {0}", profile);
                    }
