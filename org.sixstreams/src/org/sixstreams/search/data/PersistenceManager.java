...
{
	protected static Logger sLogger = Logger.getLogger(PersistenceManager.class.getName());
	...
	public void close()
	{
		// other engine instances should be cleaned up as well
	...
	{
		sLogger.info("Delete objects by query " + query);
	...
	{
		sLogger.fine("Search " + clazz.getName() + " with query " + query);
	...
	{
		sLogger.fine("runQuery " + so.getName() + " with query " + queryMetaData);
	...
	public Object toObject(Document doc, String className, List<String> rules)
	{
		...
		{
			sLogger.severe("Class must have a default constrcutor - " + className);
		}
		return obj;
	}
