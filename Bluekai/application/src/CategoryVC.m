...
-(void) showTrend:(int) categoryId
{
    trend.sql = [NSString stringWithFormat:
                 @"select to_char(created_at, 'MM/DD') created_at, inventory from bk_inventory_view where category_id = %d and site_id = -1 and sysdate - created_at < 11 order by created_at", categoryId];
    //NSLog(@"%@", trend.sql);
