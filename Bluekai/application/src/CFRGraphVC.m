...
- (void)viewDidLoad {
    ...
    graphview.sql = @"select users_run, users_found, users_wasted, to_char(start_time, 'MM/DD') TIME from bk_cfr_view where start_time between sysdate -7 and sysdate order by start_time";
    graphview.limit = 150;
    ...
    tagged.sql = @"select total_tags,total_won, to_char(start_time, 'MM/DD') TIME from bk_cfr_view where start_time between sysdate -7 and sysdate order by start_time";
    tagged.limit = 150;
