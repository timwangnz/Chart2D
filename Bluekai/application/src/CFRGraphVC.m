...
- (void)updateChart
{
    ...
    graphview.sql = [NSString stringWithFormat: @"select users_run, users_found, users_wasted, to_char(start_time, 'MM/DD') TIME from bk_cfr_daily_view where start_time between sysdate-%d and sysdate order by start_time",
                  self.days == 0 ? 1 : self.days];
    ...
    if (self.days == 1)
        ...
        tagged.sql = [NSString stringWithFormat: @"select total_tags, total_won, to_char(start_time, 'MM/DD') TIME from bk_cfr_daily_view where start_time between sysdate-%d and sysdate order by start_time",
                      self.days == 0 ? 1 : self.days];
    tagged.limit = 164;
